<template>
	<div
		class="bg-light min-vh-100 d-flex flex-row align-items-center position-relative"
	>
		<CContainer>
						...
						<CCard class="p-5">
							<CCardBody>
								<CCardTitle class="text-center"
									>Iniciar Sesi&oacute;n</CCardTitle
								>
								<CForm
	...
	export default {
		...
		methods: {
			...
			filterFiscalYears(data) {
				this.loginInfo = data;
				const fiscalYearListNoClosed = data.fiscalListYears.filter(
					(fy) => fy.estatus?.toLowerCase() !== 'cerrado',
				);
				const fiscalYearListCurrent = fiscalYearListNoClosed.filter(
					(fy) => fy.estatus?.toLowerCase() !== 'actual',
				);

				if (
					fiscalYearListNoClosed.length === 1 ||
					data.fiscalListYears.length === 1
				) {
					return this.setFiscalYearToBeUse(
						fiscalYearListNoClosed[0] ?? data.fiscalListYears[0],
					);
				}

				if (
					fiscalYearListCurrent.length === 1 &&
					data.currentFiscalYearId === fiscalYearListCurrent?.[0].id
				) {
					return this.setFiscalYearToBeUse(fiscalYearListCurrent[0]);
				}

				this.isLoading = false;
				this.fiscalYearSelectableList = fiscalYearListNoClosed.sort(
					(fy1, fy2) => fy2.anio - fy1.anio,
				);
			},
