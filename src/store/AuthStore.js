...
export const useAuthStore = defineStore('Auth', () => {
	...
	async function signIn(user) {
		return Api.Login(user).then((response) => response.data.data);
	}
	...
	function setLoginInfo(loginInfo) {
		authInfo.value = {
			...loginInfo,
			serverConfig: {
				...loginInfo.serverConfig,
				sessionTimeLimit: loginInfo.serverConfig.sessionTimeLimit
					? Number(loginInfo.serverConfig.sessionTimeLimit)
					: 300,
			},
		};
