		...
		<div class="container-fluid flex-column mb-5">
			...
			<div class="gridy">
				<div class="w-100 h-100 p-3 border border-dark">
					<div
						class="row"
						v-for="(data, index) in datosGenerales"
						:key="index"
					>
						<div class="col-6 fw-bold tamanoLetra">
							<label for="datosGenerales">
								{{ data.label }}:
							</label>
						</div>
						<p
				...
				<div class="w-100 h-100 p-3 border border-dark">
					<div
						class="row"
						v-for="(data, index) in datosLaborales"
						:key="index"
					>
						<div class="col-6 fw-bold tamanoLetra">
		...
		methods: {
			...
			getEmpleadoById(id) {
				Api.getEmpleadoByID(id).then((response) => {
					...
					this.empleado.fechaIngreso = this.formatDate(
					...
					}/api/files/public/${this.empleado.idImagenPerfil ?? -1}`;
			...
			lookInfo(param) {
				return (
					param
						.split('.')
						.reduce((acc, item) => acc?.[item], this.empleado) ??
					'&nbsp;'
				);
			},
		},
