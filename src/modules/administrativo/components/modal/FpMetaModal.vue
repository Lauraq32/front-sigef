<template>
	...
	>
		...
		<CModalBody>
			...
			>
				...
				<div class="row">
					<p>
						...
						{{ fpMeta.estructuraProgramaticaId }}
					</p>
					<div class="row">
						<p>
					...
					<div class="row">
						<p>
							...
							{{ estructura.ctgFuncionId }}
						</p>
					</div>
					<div class="row">
						<p>
							<strong>Unidad Responsable: </strong>
							{{ estructura.unidadRespon }}
						</p>
					</div>
				</div>
				...
				<CFormTextarea
					rows="4"
					v-model="fpMeta.descripcion"
					required
				></CFormTextarea>
				<div
					class="col-12 border-bottom my-3 d-flex justify-content-between"
				>
					<CButton
						size="sm"
						color="secondary"
						class="mb-3"
						@click="() => (showModalFpMetaDetalle = true)"
						>Agregar Detalle
					</CButton>
				</div>
				<div>
					<CSmartTable
						...
						:activePage="1"
						header
						:items="fpMeta.fpMetaDetalles"
						:columns="tableColumnsFpMetaDetalles"
						:itemsPerPage="5"
						columnSorter
			...
			<FpMetaDetalleModal
				:modalFpMetaDetalle="showModalFpMetaDetalle"
				@closeModalFpMetaDetalle="closeFpMetaDetalleModal"
				@getFpMetaDetalles="addDetalleToFpMetaDetalle"
			/>
			<SelectMestProgModal
				:showModal="mestProgDialog"
...
<script>
	...
	import { CFormTextarea, CSmartTable } from '@coreui/vue-pro';
	...
	export default {
		data: function () {
			return {
				tableColumnsFpMetaDetalles: [
					...
					{
						key: 'estimadoAnioActual',
						label: 'Estimado Año Actual',
						_style: { width: '20%' },
					},
					{
						key: 'programadoAnioSiguiente',
						label: 'Programado Año Siguiente',
						_style: { width: '25%' },
					},
					{
						key: 'show_details',
		...
		methods: {
			...
			removeAFpMetaDetalle(value) {
				if (value.id) {
					AdministrativoApi.deleteFpMetaDetalle(value.id)
						.then(() => {
							this.fpMeta.fpMetaDetalles =
								this.fpMeta.fpMetaDetalles.filter(
									(fpMetaDetalle) =>
										fpMetaDetalle.id !== value.id,
								);
						})
						.catch(({ data }) => {
							this.show({
					...
					this.fpMeta.fpMetaDetalles.splice(
						this.fpMeta.fpMetaDetalles.indexOf(value),
						1,
					);
				}
			},
			saveFpMeta() {
				if (
					!Array.isArray(this.fpMeta.fpMetaDetalles) ||
					!this.fpMeta.fpMetaDetalles.length
				) {
					this.show({
				...
				if (!this.fpMeta.id) {
					AdministrativoApi.createFpMetaAndDetalle(this.fpMeta)
						.then(({ data }) => {
							this.show({
				...
				AdministrativoApi.editFpMetaAndDetalle(
					this.fpMeta.id,
					this.fpMeta,
				)
					.then(({ data }) => {
