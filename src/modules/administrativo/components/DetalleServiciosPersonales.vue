<template>
		...
		<div class="p-2">
			<div class="row">
				<div class="col-6">
					...
					<div class="">
						<div class="position-relative mb-4 w-100">
							<input
								v-model="
									servicioPersonales.estructuraProgramaticaId
								"
				...
				<div class="col-6">
					...
					<div class="">
						<div class="position-relative mb-4 w-100">
							<input
								v-model="servicioPersonales.clasificadorId"
								class="form-control padding-input"
			...
			<div class=" " style="text-align: right">
				<CButton
					size="sm"
					color="primary"
					@click="showDetalleServiciosPersonalesDialog"
					>Agregar Detalle
				</CButton>
			</div>
		</div>
			...
			>
				...
				<template #AnualSolicitado="{ item }">
					<td>
						{{
							formatPrice(
								item.mensualsolicitado *
									item.cantidadsolicitada,
							)
						}}
					</td>
				</template>
				<template #show_details=""> </template>
			</CSmartTable>
		</div>
		<CModalFooter>
			<CButton
			...
			<div class=" ">
				<CButton color="primary" class="" @click="saveServicios"
					>Guardar
				</CButton>
			</div>
		</CModalFooter>
	...
	<SelectClasificador
		:showModal="clasificadorDialog"
		@closeClasificador="closeDialogClasificador"
	/>
	...
	<DetalleServiciosPersonales
		:showModal="DetalleServiciosPersonalesDialog"
	...
	export default {
		...
		methods: {
			...
			saveServicios() {
				if (this.servicioPersonales.id) {
					Api.putFpServicioPersonalById(
						this.servicioPersonales.id,
						this.servicioPersonales,
					)
						.then(() => {})
						.then(() => {
				...
				Api.postFpServicioPersonal(this.servicioPersonales)
					.then(() => {
			...
			saveDetalle(data) {
				...
				const list = Array.from({ length: 6 }, (_, i) => {
					...
					return filteredKeys.reduce((acc, key) => {
						acc[key.replace(term, '').toLocaleLowerCase()] =
							data[key];
						acc['Categorias'] = categoriasServicios[i];
						acc['AnualActual'] = acc['cantidad'] * acc['mensual'];
						acc['AnualSolicitado'] =
							acc['cantidadsolicitada'] *
							acc['mensualsolicitado'];
						return acc;
					}, {});
				});
				list.map((item, index) => {
					index = index + 1;
					this.servicioPersonales[`go0${index}Cantidad`] =
						item['cantidad'];
					this.servicioPersonales[`go0${index}Mensual`] =
						item['mensual'];
					this.servicioPersonales[`go0${index}CantidadSolicitada`] =
						item['cantidadsolicitada'];
					this.servicioPersonales[`go0${index}MensualSolicitado`] =
						item['mensualsolicitado'];
				});
			...
			closeMestProgDialog(data) {
				if (data) {
					this.servicioPersonales.estructuraProgramaticaId =
						data.numero;
		...
		watch: {
			servicioPersonalesProps(value) {
				...
				this.servicioPersonales.clasificadorId =
					value.clasificador.clasifica;
