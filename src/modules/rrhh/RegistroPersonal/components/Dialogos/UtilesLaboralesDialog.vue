			...
			<div class="table-headers">
				<div class="d-inline p-2">
					<CButton
						...
						@click="
							() => {
								showEventoInventarioDialog = true;
							}
						"
					>
				...
				<template #estatus="{ item }">
					<td>
						<CBadge
							...
							:color="determineColor(item.estatus)"
							>{{ item.estatus }}</CBadge
						>
					</td>
				</template>
				...
				<template #controllers="{ item }">
					<td>
						<CDropdown>
							...
							<CDropdownMenu>
								<CDropdownItem
									@click="() => handlerAccion(item, 'print')"
									v-if="item.estatus !== 'Cancelado'"
									>Imprimir</CDropdownItem
								>
								<CDropdownItem
									@click="() => handlerAccion(item, 'editar')"
									v-if="item.estatus == 'Registrado'"
									>Editar</CDropdownItem
								>
								<CDropdownItem
									@click="() => handlerAccion(item, 'detail')"
									>Ver Detalle</CDropdownItem
								>
								<CDropdownItem
									@click="() => handlerAccion(item, 'cancel')"
									v-if="
										!['Cancelado', 'Entregado'].includes(
											item.estatus,
										)
									"
									>Cancelar</CDropdownItem
								>
								<CDropdownItem
									@click="
										() => handlerAccion(item, 'approved')
									"
									v-if="
										!['Cancelado', 'Entregado'].includes(
											item.estatus,
									...
									"
									>Marcar como entregado</CDropdownItem
								>
							</CDropdownMenu>
						</CDropdown>
					</td>
	...
	export default {
		...
		methods: {
			...
			loadUtilsByEmployeeId() {
				Api.getUtilesLaborales(this.employeeInfo.id).then(
					({ data: { data } }) => {
			...
			loadUtilDetailByUtil(util) {
				Api.getUtilLaboralEventos(util.id).then(
					({ data: { data } }) => {
