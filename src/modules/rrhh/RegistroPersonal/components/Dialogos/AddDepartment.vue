			...
			<CCardBody>
				...
				>
					...
					<CContainer class="mt-2">
						<CRow>
							<CCol :md="6">
								<CFormLabel
									>Nombre Depto. &oacute;
									N&oacute;mina</CFormLabel
								>
								<CFormInput
									required
									v-model="newDepartment.nombre"
								>
								</CFormInput>
							...
							<CCol :md="6">
								...
								<CFormSelect
									required
									v-model="newDepartment.grupoNominaId"
								>
									<option value="0">
										Selecionar Grupo N&oacute;mina
									</option>
									<option
					...
					<div>
						...
						<CFormSelect
							required
							v-model="newDepartment.cuentaBancoId"
						>
					...
					<div>
						...
						<CFormInput
							required
							v-model="newDepartment.estructura"
							disabled
						>
						</CFormInput>
					</div>
							...
							<CRow>
								<CCol :md="3">
									...
									<CFormSelect
										required
										v-model="
											newDepartment.ctgClasificadorId
										"
								...
								<CCol :md="3">
									<CFormLabel
										>Fuente Financiamineto</CFormLabel
									>
									<CFormInput
								...
								<CCol :md="3">
									...
									<CFormInput
										v-model="
											newDepartment.ctgFuenteEspecificaId
										"
										required
										maxlength="4"
										@keypress="onlyNumber"
								...
								<CCol :md="3">
									<CFormLabel
										>Organismo Financiero</CFormLabel
									>
									<CFormInput
										v-model="
											newDepartment.ctgOrganismoFinanciadorId
										"
										required
										maxlength="4"
										@keypress="onlyNumber"
								...
								<CCol
									:md="12"
									v-if="paymentClasificatorMessage"
									class="mt-2"
								>
									<CAlert color="danger">{{
						...
						<CContainer class="mt-4">
							...
							<CRow>
								<CCol :md="3">
									...
									<CFormInput
										v-model="
											newDepartment.ctgClasificadorRegaliaId
										"
								...
								<CCol :md="3">
									...
									<CFormInput
										v-model="
											newDepartment.ctgFuenteRegaliaId
										"
										maxlength="2"
										@keypress="onlyNumber"
									></CFormInput>
								...
								<CCol :md="3">
									...
									<CFormInput
										v-model="
											newDepartment.ctgFuenteEspecificaRegaliaId
										"
										maxlength="4"
										@keypress="onlyNumber"
									></CFormInput>
								...
								<CCol :md="3">
									...
									<CFormInput
										v-model="
											newDepartment.ctgOrganismoFinanciadorRegaliaId
										"
										maxlength="4"
										@keypress="onlyNumber"
									>
								...
								<CCol
									:md="12"
									v-if="regaliaClasificatorMessage"
									class="mt-2"
								>
									<CAlert color="danger">{{
	...
	export default {
		...
		methods: {
			...
			handleProgramaChange(idPrograma) {
				...
				const programa = this.programas.find(
					(x) => x.id === this.newDepartment.programaDivisionId,
				);
			...
			handleClasificadorChange({ target }) {
				const selected =
					this.clasificadoresFiltrados[target.selectedIndex - 1];
				if (selected) {
		...
		mounted() {
			...
			departmentService
				...
				.then(
					(response) =>
						(this.estructurasProgramaticas = response.data.data),
				);
			departmentService
			...
			departmentService.getClasificadores().then((response) => {
				const clasificadores = response.data?.data
					?.filter((clasificator) =>
						clasificator?.clasifica?.toString().match(/^(2)/g),
					)
					?.filter((x) =>
						clasificadoresPermitidos.includes(x.clasifica),
					);

				const uniqueClasificadores = clasificadores.reduce(
					(obj, item) => {
						obj[
							`${item.clasifica}-${item.source.mestProgId}-(${item.ctgFuenteId}/${item.ctgFuenteEspecificaId}/${item.ctgOrganismoFinanciadorId})`
						] = item;
						return obj;
					},
					{},
			...
			departamento() {
				if (this.departamento) {
					this.newDepartment = {
						...
						programaDivisionId:
							this.departamento.programaDivision?.id,
						...
						ctgOrganismoFinanciadorId:
							this.departamento.organismoFinanciador?.id,
						ctgClasificadorRegaliaId:
							this.departamento.clasificadorRegalia?.id,
						ctgOrganismoFinanciadorRegaliaId:
							this.departamento.organismoFinanciadorRegalia?.id,
						ctgFuenteId: this.departamento.fuente?.id,
						ctgFuenteEspecificaId:
							this.departamento.fuenteEspecifica?.id,
