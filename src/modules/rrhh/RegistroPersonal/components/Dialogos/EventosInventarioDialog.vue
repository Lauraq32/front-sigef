				...
				<CCardBody class="mx-4">
					...
					<CCol class="row">
						<CFormLabel
							for="empleadoNombre"
							class="form-label col-3"
							>Nombre Empleado</CFormLabel
						>
						<strong class="col-9"
							>{{ empleado?.nombre }}
							{{ empleado?.apellido }}</strong
						>
					</CCol>
					<hr />
					<CCol>
						<CFormLabel
							for="postEvento.autorizadoPor"
							class="form-label"
					...
					<CCol>
						<CFormLabel for="postEvento.observacion"
	...
	export default {
		...
		computed: {
			initialDate: {
				...
				set(value) {
					return (this.postEvento.fecha = new Date(
						`${value}T00:00:00`,
					));
				},
			},
		...
		methods: {
			...
			removeUtil(util) {
				this.postEvento.utiles = this.postEvento.utiles.filter(
					(u) => u.inventarioId !== util.inventarioId,
				);
			},
			...
			onMovimientoSave(movement) {
				const foundSimilar = this.postEvento.utiles.filter(
					(u) => u.inventarioId === movement.inventarioId,
				);
			...
			sendData() {
				...
				if (
					...
					this.util.id
						? this.editarEvent({ ...this.postEvento }, this.util.id)
						: this.submitEventos({
								...this.postEvento,
		...
		watch: {
			...
			util(value) {
				if (this.showModal) {
					...
					this.postEvento.uitles = this.postEvento.utiles.forEach(
						function (utiles) {
