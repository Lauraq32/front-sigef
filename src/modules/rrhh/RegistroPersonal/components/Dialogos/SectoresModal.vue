<template>
	<CModal :visible="newSectorModal" backdrop="static" style="width: 25%">
		<CModalHeader>
		...
		<CModalFooter>
			...
			<CButton color="primary" @click="saveSector"> Guardar </CButton>
		</CModalFooter>
	</CModal>
</template>
<script>
	...
	export default {
		...
		watch: {
			sectorId(newId) {
				if (newId) {
					Api.getSectorById(newId).then((response) => {
