	...
	>
		...
		<CModalBody>
			<CCardBody>
				...
				>
					<CCol :md="4">
						<CFormLabel for="employeeInfoId"
							>C&oacute;digo Empleado</CFormLabel
						>
						<CFormInput
							required
							id="employeeInfoId"
					...
					<CCol :md="12">
						<CFormLabel for="courseName"
							>Nombre del Curso</CFormLabel
						>
						<CFormInput
							required
							id="courseName"
					...
					<CCol :md="12">
						<CFormLabel for="courseTime"
							>Duraci&oacute;n del Curso en Horas</CFormLabel
						>
						<CFormInput
							v-model="education.courseTime"
							required
							id="courseTime"
							@keypress="onlyNumber"
						>
					...
					<CCol :md="12">
						<CFormLabel for="courseRecord"
							>R&eacute;cord del Curso</CFormLabel
						>
						<CFormTextarea
							v-model="education.courseRecord"
							id="courseRecord"
							required
						>
						</CFormTextarea>
				...
				<CSmartTable
					...
					:columns="columns"
					:itemsPerPage="5"
					:sorterValue="{ column: 'courseName', state: 'asc' }"
					pagination
				>
					...
					<template #courseTime="{ item }">
						<td class="py-2">{{ item.courseTime }} horas</td>
					</template>
...
<script>
	...
	export default {
			...
			saveEducation(event) {
				...
				if (form.checkValidity() === true) {
					if (this.educationId == null) {
						Api.postEmployee(this.education)
							.then(() => {
								this.show({
						...
						Api.putEmployee(this.education, this.educationId)
							.then(() => {
								this.show({
									content:
								...
								setTimeout(
									this.listarEducation(this.employeeInfo.id),
									500,
								);
								this.profesionFormValidated = false;
								this.clearModal();
		...
		watch: {
			employeeInfo(employeeInfoVal) {
				if (employeeInfoVal.id && this.showModal) {
					this.listarEducation(employeeInfoVal.id);
				}
			},
		},
