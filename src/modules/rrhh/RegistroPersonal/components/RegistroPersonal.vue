    ...
    <template #show_details="{ item }">
      <CDropdown>
        ...
        <CDropdownMenu>
          ...
          <CDropdownItem @click="EvaluationReport()">Evaluación</CDropdownItem>
          <CDropdownItem>Eventualidad</CDropdownItem>
          <CDropdownItem @click="() => {
            lgDemo4 = true
          }
            ">Acci&aacute;n Personal</CDropdownItem>
        </CDropdownMenu>
      </CDropdown>
    </template>
...
export default {
  ...
  methods: {
    ...
    EvaluationReport(id) {
      this.$router.push({ path: 'pages/evaluacion', query: { idEmployee: id } })
    },
    submitForm() {
      if (this.id != null) {
