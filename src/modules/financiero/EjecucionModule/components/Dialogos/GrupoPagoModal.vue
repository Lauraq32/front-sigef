	...
	>
		...
		<CModalBody>
			<CCardBody>
				...
				>
					<CCol>
						<CFormLabel for="postGrupo.descripcion"
							>Descripci&oacute;n</CFormLabel
						>
						<CFormInput
							id="postGrupo.descripcion"
							v-model="postGrupo.descripcion"
							@keypress="onlyLetter"
							maxlength="20"
							required
						/>
					...
					<CCol>
						<CFormLabel for="postGrupo.fechaInicio"
							>Fecha Inicio</CFormLabel
						>
						<CFormInput
							type="date"
							id="postGrupo.fechaInicio"
							v-model="fechaInicio"
							required
						/>
						<CFormFeedback invalid>
					...
					<CCol>
						<CFormLabel for="postGrupo.fechaFin"
							>Fecha Fin</CFormLabel
						>
						<CFormInput
							type="date"
							id="postGrupo.fechaFin"
							:disabled="!postGrupo.id"
							v-model="fechaFin"
						...
						<CFormFeedback
							invalid
							:style="{
								display: fechaHataValidation ? 'flex' : 'none',
							}"
						>
			...
			fechaInicio: {
				get() {
					...
					if (
						...
						this.postGrupo.fechaInicio?.toString() !==
							'Invalid Date'
					) {
