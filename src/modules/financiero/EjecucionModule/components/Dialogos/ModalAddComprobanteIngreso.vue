	...
	>
		...
		<CModalBody>
							...
							<div class="row">
								<div class="col-6">
									...
									<CFormInput
										...
										v-model="
											newIncome.codigoIngresoTalonario
										"
									>
									</CFormInput>
								</div>
						...
						<div class="col-6">
							<div>
								...
								<div class="position-relative">
									<CFormInput
										...
										v-model="newIncome.contribuyente.nombre"
							...
							<div class="row">
								...
								<div class="col-6">
									...
									<CFormInput
										...
										required
										v-model="
											newIncome.contribuyente.documento
										"
										id="documento"
									>
									</CFormInput>
							...
							<div class="row">
								<div class="col-6">
									...
									<CFormInput
										v-model="
											newIncome.contribuyente.direccion
										"
										id="direccion"
									>
									</CFormInput>
								...
								<div class="col-6">
									<CFormLabel for="Teléfono"
										>Teléfono:</CFormLabel
									>
									<CFormInput
										class="form-control"
										v-model="
											newIncome.contribuyente.telefono
										"
										id="Teléfono"
										@keypress="onlyNumber"
									/>
					...
					<div class="mb-4">
						<CCol :md="12">
							...
							<CFormTextarea
								style="resize: none"
								required
								v-model="newIncome.detalle"
								id="Detalle"
							></CFormTextarea>
							<CFormFeedback invalid>
								Campo requerido</CFormFeedback
							>
						</CCol>
					</div>
				</CForm>
				...
				>
					...
					<div class="row border-bottom mb-3">
						<div class="col-12 row align-items-center my-2">
							...
							>
								<span class="col-form-label fw-bold p-2"
									>Sub total:</span
								>
								{{ subTotal }}
							</div>
			...
			<CSmartTable
				...
				:activePage="1"
				header
				:items="newIncome.detalleRegistroIngresos"
				:columns="columns"
				:footer="footerItem"
				columnFilter
				:itemsPerPage="5"
...
<script>
	...
	export default {
		...
		methods: {
			...
			addDetalle() {
				...
				if (
					...
					this.newIncome.detalleRegistroIngresos = [
						{ ...this.detallePost },
						...this.newIncome.detalleRegistroIngresos,
					];
					this.clearDetalle();
			...
			addComprobanteIngreso() {
				this.$emit('add-comprobante', { ...this.newIncome });
			},

			sendData() {
				this.isFormEventTypeValidated = false;
				if (!this.newIncome.detalleRegistroIngresos.length) {
			...
			selectItemEventHandler(payload) {
				this.newIncome.contribuyente = payload;
			},
			...
			changeDocument() {
				this.cedulaMax =
					this.newIncome.contribuyente.tipoDocumento === 'cedula'
						? 11
						: 15;
				this.newIncome.contribuyente.documento = null;
			},

			checkDocument(e) {
				if (this.newIncome.contribuyente.tipoDocumento === 'Cedula') {
					onlyNumber(e);
					this.cedulaMax = 11;
				}
			},
			...
			selectClasificator(infoClasificador) {
				if (infoClasificador) {
					this.detallePost.ctgClasificadorId =
						infoClasificador.clasifica;
			...
			eliminarObjeto(detalle) {
				this.$swal({
					...
					if (result.isConfirmed) {
						if (this.newIncome.detalleRegistroIngresos) {
							this.newIncome.detalleRegistroIngresos =
								this.newIncome.detalleRegistroIngresos.filter(
									(d) => d.id !== detalle.id,
								);
						}
					}
		...
		computed: {
			...
			fecha: {
				get() {
					let date = this.newIncome.fecha;
					if (
						this.newIncome.fecha !== null &&
						this.newIncome.fecha?.toString() !== 'Invalid Date'
					) {
						if (typeof this.newIncome.fecha === 'string') {
							date = new Date(this.newIncome.fecha);
				...
				set(value) {
					return (this.newIncome.fecha = new Date(
						`${value}T00:00:00`,
					));
				},
			},
		...
		watch: {
			...
			ingresoPost(neeValue) {
				this.newIncome = { ...(neeValue ?? {}) };
			},
		},
