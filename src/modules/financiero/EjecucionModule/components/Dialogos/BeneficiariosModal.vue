		...
		<CModalBody>
			<CCardBody>
				<CForm
					...
					novalidate
					:validated="formBeneficiariosValidate"
				>
					<div>
						<div>
							<div class="row">
					...
					<div class="row">
						<CCol class="col-6">
							...
							<CurrencyInput
								...
								id="presupuestoBco1"
								:disabled="
									postBeneficiario.tipo !== 'pensionado' &&
									postBeneficiario.tipo !== 'subvencion'
								"
					...
					<CModalFooter>
						...
						<CButton
							...
							v-on:click="createBeneficiarios"
						>
							Guardar
						</CButton>
					</CModalFooter>
...
<script>
	...
	export default {
			...
			fechaInicio: {
				get() {
					...
					if (
						...
						this.postBeneficiario.ingreso?.toString() !==
							'Invalid Date'
					) {
				...
				set(value) {
					this.postBeneficiario.ingreso = new Date(
						`${value}T00:00:00`,
					);
				},
			},
		...
		methods: {
			...
			changeDocument() {
				this.IdCodMaxLen =
					this.postBeneficiario.tipoDcto === 'cedula' ? 11 : 15;
		...
		watch: {
			...
			'postBeneficiario.tipo': function () {
				if (
					(this.postBeneficiario.tipo !== 'pensionado' ||
						this.postBeneficiario.tipo !== 'subvencion') &&
					!this.postBeneficiario.id
				) {
					this.postBeneficiario.mensual = 0;
					this.postBeneficiario.recomienda = 0;
				}
