<template>
	...
	>
		...
		<template #estatus="{ item }">
			<td>
				<CBadge
					class="d-block mt-1"
					:color="item.estatus ? 'success' : 'danger'"
	...
	<BeneficiariosModal
		:beneficiarioToUpdate="beneficiarioToUpdate"
		:showModal="showBeneficiarioModal"
		@postBeneficiarios="submitForm"
				...
				Api.getBeneficiarios().then((response) => {
					this.Beneficiarios = response.data.data.map((x) => {
						return {
							...
							recomienda:
								x.recomienda === null ? '' : x.recomienda,
						};
					});
				});
			},
			...
			deleteBeneficiario(item) {
				this.$swal({
					...
					if (result.isConfirmed) {
						Api.deleteBeneficiarios(item.id)
							.then(() => {
								this.show({
