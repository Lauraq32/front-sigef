<template>
		...
		<template #show_details="{ item }">
			<td class="py-2">
				<CButton
					...
					square
					size="sm"
					@click="updateBeneficiaryGroup(item)"
				>
					Editar
				</CButton>
			</td>
	...
	<beneficiarioGrupoModal
		:beneficiarioxGrupoModal="beneficiarioxGrupoModal"
		@close-modal="closeModal"
		@post-Beneficiariogrupo="saveBeneficiarioGrupo"
		:beneficiarioGroupToUpdate="beneficiarioGroupToUpdate"
	/>
</template>
<script>
	...
	export default {
		...
		data: () => {
			return {
				...
				columns: [
					...
					{
						key: 'documento',
						label: 'Cédula/Pasaporte/RNC',
						_style: { width: '14%' },
					},
					{
						key: 'grupoCompensacion',
						label: ' Grupo de Pago',
						_style: { width: '20%' },
					},
					{
						key: 'cargoBeneficiario',
						label: 'Cargo',
						_style: { width: '14%' },
					},
		...
		methods: {
			...
			saveBeneficiarioGrupo(payload) {
				if (payload.id) {
					Api.putBeneficiarioGrupo(payload.id, payload)
						.then(() => {
							this.show({
