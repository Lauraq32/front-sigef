<template>
	...
	>
		...
		<CModalBody>
				...
				<div class="row">
						...
						>
							<div class="row">
								<CCol :md="8" class="mb-3">
									<CFormLabel for="estructuraProgramatica"
										>Programa o Proyecto</CFormLabel
									>
									<div class="position-relative">
										<input
											v-model="
												detalleRegistroGasto.estructuraProgramatica
											"
											ref="name"
											disabled
											required
										...
										>
											<CIcon
												icon="cisSearch"
												size="xl"
												v-on:click="showMestProgDialog"
											/>
										</span>
									</div>
								</CCol>
								<CCol :md="4" class="text-center">
									<CFormLabel for="etapa" class="d-block"
										>Etapa:</CFormLabel
									>
									<span class="fw-bold">{{
										registroGasto.etapa
								...
								<CCol :md="6">
									...
									<CFormInput
										...
										:keypress="onlyNumber"
										v-model="
											detalleRegistroGasto.clasificadorId
										"
										id="nombre"
										required
									/>
								...
								<CCol :md="6" class="border text-center">
									<CFormLabel
										for="balanceDisponible"
										class="font-weight-bold"
										>Balance Disponible</CFormLabel
									>
									<span
										class="d-block"
										style="font-weight: bold"
										>{{
											formatPrice(balanceDisponible)
										}}</span
									>
								</CCol>
								<CCol
									:md="12"
									class="mt-3 p-2 bg-light bg-clasificador text-center"
								>
									<span class="fw-bold">
										{{ nombreClasificador }}
									</span>
								...
								<CCol :md="6" class="mt-3">
									...
									<VueNumberFormat
										v-model:value="
											detalleRegistroGasto.montoBruto
										"
										@change="setMontoBruto"
										type="text"
										step="any"
								...
								<hr class="mt-4" />
								<fieldset
									:disabled="registroGasto.etapa !== 'Pagado'"
								>
											...
											<v-select
												:disabled="
													...
													this.detalleRegistroGasto
														.montoBruto == 0
												"
												v-model="tipoRetencionObj"
												:options="tipoRetencionesList"
												@input="setRetencion"
											></v-select>
										</CCol>
										<CCol :md="2">
								...
								<CCol :md="12">
									<CSmartTable
										...
										header
										:items="
											detalleRegistroGasto.detalleRetencion
										"
										:columns="RetencionColumn"
										columnFilter
										:footer="footerItemRetenciones"
										:itemsPerPage="10"
										columnSorter
										:sorterValue="{
											column: 'nombres',
					...
					<div class="col-8">
						<CCol :md="12">
							<h5>
								Cuenta Banco:
								<span class="fw-bold">{{ cuentaBanco }}</span>
							</h5>
							...
							>
								...
								<template #presupuestoBco="{ item }">
									<td>
										{{
											item.presupuestoBco != 0
												? formatPrice(
														item.presupuestoBco,
												  )
												: ''
								...
								<template #devengadoEjecutado="{ item }">
									<td>
										{{
											item.devengadoEjecutado != 0
												? formatPrice(
														item.devengadoEjecutado,
												  )
												: ''
										}}
									</td>
								</template>
								<template #devengadoDisponible="{ item }">
									<td>
										{{
											item.devengadoDisponible != 0
												? formatPrice(
														item.devengadoDisponible,
												  )
												: ''
										}}
									</td>
								</template>
								<template #pagadoEjecutado="{ item }">
									<td>
										{{
											item.pagadoEjecutado != 0
												? formatPrice(
														item.pagadoEjecutado,
												  )
												: ''
										}}
									</td>
								</template>
								<template #pagadoDisponible="{ item }">
									<td>
										{{
											item.pagadoDisponible != 0
												? formatPrice(
														item.pagadoDisponible,
												  )
												: ''
										}}
									</td>
						...
						<CCol :md="12" class="border p-2">
							<h4 class="text-center mb-3">
								Fondos del Gasto (Cta./Banco) en los que el
								clasificador seleccionado fue presupuestado
							</h4>
							<div class="row">
								<div class="col-2">
									<div class="row">
								...
								<div class="col-10">
									...
									<div class="row">
										<div
											class="col-3 text-center fw-semibold"
										>
											{{ cuentasClasificadores[0] }}
										</div>
										<div
											class="col-3 text-center fw-semibold"
										>
											{{ cuentasClasificadores[1] }}
										</div>
										<div
											class="col-3 text-center fw-semibold"
										>
											{{ cuentasClasificadores[2] }}
										</div>
										<div
											class="col-3 text-center fw-semibold"
										>
											{{ cuentasClasificadores[3] }}
										</div>
									</div>
								</div>
							</div>
			...
			<MestProgDialog
				@closeMestProg="closeMestProgDialog"
				:showModal="MestProgDialogProp"
			/>
		</CModalBody>
		<CModalFooter>
			<CButton
			...
			<CButton color="info" @click="saveDetalles"
				>Guardar Detalle
			</CButton>
		</CModalFooter>
	</CModal>
	...
	export default {
		...
		methods: {
			...
			closeMestProgDialog(data) {
				...
				this.detalleRegistroGasto.estructuraProgramatica =
					data.mestProgId;
			...
			guardarRetencion() {
				if (this.tipoRetencionObj.code) {
					this.detalleRegistroGasto.montoNeto =
						this.detalleRetencion.baseImponible;
					this.detalleRetencion.tipoRetencionId =
						this.tipoRetencionObj.code;
					this.detalleRetencion.nombreRetencion =
						this.tipoRetencionObj.label;
					this.detalleRetencion.montoAplicado =
						this.detalleRegistroGasto.montoBruto;
					this.detalleRegistroGasto.detalleRetencion = [
						{ ...this.detalleRetencion },
						...this.detalleRegistroGasto.detalleRetencion,
					];
					this.footerItemRetenciones[2].label = this.formatPrice(
						Number(this.detalleRetencion.valorAplicado) +
							Number(this.footerItemRetenciones[2].label),
					);
					this.footerItemRetenciones[1].label = this.formatPrice(
						Number(this.detalleRegistroGasto.montoBruto) -
							Number(this.footerItemRetenciones[2].label),
					);
					return;
				}
			...
			saveDetalle() {
				if (
					this.detalleRegistroGasto.estructuraProgramatica &&
					this.detalleRegistroGasto.clasificadorId &&
					this.detalleRegistroGasto.montoBruto
				) {
			...
			deleteRetencion(item) {
				this.detalleRegistroGasto.detalleRetencion.splice(
					this.detalleRegistroGasto.detalleRetencion.indexOf(item),
					1,
				);
				this.footerItemRetenciones[2].label = this.formatPrice(
					Number(this.detalleRetencion.valorAplicado) +
			...
			getMestProg(item) {
				Api.getRegistroGastoMesProg(item)
					.then((response) => {
						if (response.data.data.length > 1) {
							var clasificadores = response.data.data.filter(
								(item) => item.clasificador.length == 6,
							);

							var dataResponse = clasificadores.reduce(
								(acc, current) => {
									acc.push({
										...
										organismoFinanciadorId:
											current.organismoFinanciadorId,
										fuenteEspecificaId:
											current.fuenteEspecificaId,
										...
										presupuestoBco:
											current[
												`presupuestoBco${this.registroGasto.bancoId}`
											],
										OFin: `${current.fuenteId}/${current.organismoFinanciadorId}/${current.fuenteEspecificaId}`,
										totalPagadoBco:
											current[
												`totalPagadoBco${this.registroGasto.bancoId}`
											],
										variacionBco:
											current[
												`variacionBco${this.registroGasto.bancoId}`
											],
										pActual:
											current[
												`presupuestoBco${this.registroGasto.bancoId}`
										...
										devengadoEjecutado:
											this.registroGasto.etapa ==
											'Devengado'
												? current[
														`totalDevengadoBco${this.registroGasto.bancoId}`
												  ]
												: 0,
										devengadoDisponible:
											this.registroGasto.etapa ==
											'Devengado'
												...
												: 0,
										pagadoEjecutado:
											this.registroGasto.etapa == 'Pagado'
												? current[
														`totalPagadoBco${this.registroGasto.bancoId}`
												  ]
												: 0,
			...
			selectMestProg(item) {
				...
				this.detalleRegistroGasto.fuenteEspecificaId =
					item.fuenteEspecificaId;
				this.detalleRegistroGasto.organismoFinanciadorId =
					item.organismoFinanciadorId;
		...
		watch: {
			tipoRetencionObj(value) {
				...
				switch (value.operacion) {
					case 'Multiplicar':
						this.detalleRetencion.valorAplicado = value.calculada
							? this.detalleRegistroGasto.montoBruto *
							  (this.detalleRetencion.montoAplica / 100)
							: this.detalleRegistroGasto.montoBruto +
							  this.detalleRetencion.montoAplica;
						this.detalleRetencion.montoNeto =
							this.detalleRetencion.valorAplicado;
						break;
					case 'Dividir':
						this.detalleRetencion.valorAplicado = value.calculada
							? this.detalleRegistroGasto.montoBruto / 1.1
							: this.detalleRegistroGasto.montoBruto +
							  this.detalleRetencion.montoAplica;
						this.detalleRetencion.montoNeto =
							this.detalleRetencion.valorAplicado;
