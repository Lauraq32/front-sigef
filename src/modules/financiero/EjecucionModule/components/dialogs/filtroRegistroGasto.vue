  ...
  <CModal @close="() => closeFiltro()" :visible="showFiltro" backdrop="static">
    ...
    <CModalBody>
      ...
      <div class="m-2">
        ...
        <AppDateField class="form-control" v-model="filtroValue.FechaDesde" />
      </div>
      <div class="m-2">
        <CFormLabel>Fecha Hasta</CFormLabel>
        <AppDateField class="form-control" v-model="filtroValue.FechaHasta" />
      </div>
    </CModalBody>
...
export default {
  ...
  methods: {
    ...
    getServiciosRequeridos() {
      this.filtroValue.FechaHasta = new Date(`${this.fiscalYearInfo().anio}-01-01`)
