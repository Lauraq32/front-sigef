	...
	>
		...
		<CModalBody>
			<CCardBody>
							...
							<div class="row">
								<div class="col-6">
									<CCol :md="12">
										<div class="row">
											<CCol :md="6">
												<CFormLabel for="fecha"
													>Fecha</CFormLabel
												>
												<AppDateField
													class="form-control"
													v-model="
														postRegistroGasto.fecha
													"
												/>
											</CCol>

											<CCol :md="6">
												<CFormLabel for="etapa"
													>Etapa</CFormLabel
												>
												<CFormSelect
													@change="changeEtapa"
													v-model="
														postRegistroGasto.etapa
													"
													id="etapa"
													required
												>
											...
											<CCol :md="6">
												<CFormLabel
													for="fechaResolucion"
													>Fecha
													Resoluci&oacute;n</CFormLabel
												>
												<AppDateField
													class="form-control"
													v-model="
														postRegistroGasto.fechaResolucion
													"
												/>
											</CCol>
											...
											<CCol :md="6">
												<CFormLabel for="nombre"
													>Comprobante
													Modificado</CFormLabel
												>
												<CFormInput
													:disabled="
														disabledComprobante
									...
									<CCol :md="12">
										<div class="row">
											...
											<CCol :md="12">
												<CFormLabel
													for="conceptoGastoId"
													>Por Concepto de</CFormLabel
												>

												<CFormSelect
													required
													v-model="
														postRegistroGasto.conceptoGastoId
													"
												>
													<option :key="0">
														Selecionar concepto
													...
													<option
														v-for="concepto in conceptosGasto"
														:value="`${concepto.id}`"
														:key="concepto.id"
													>
														{{
											...
											<CCol :md="12">
												...
												<CFormSelect
													required
													v-model="
														postRegistroGasto.bancoId
													"
													@change="getCuentabanco"
												>
													<option :key="0">
														Selecionar Cuenta de
														banco
													</option>
													<option
								...
								<div class="col-12">
									...
									<textarea
										v-model="postRegistroGasto.detalle"
										name=""
										id="detalle"
										style="width: 100%"
									>
				...
				<CButton color="secondary" @click="openCodificacionGasto"
					>Agregar detalle</CButton
				>
				...
				<CSmartTable
					...
					:activePage="1"
					header
					:items="postRegistroGasto.detalleRegistroGastos"
					:columns="detalleGastoColumns"
					columnFilter
					:footer="footer"
					itemsPerPageSelect
					:itemsPerPage="5"
				...
				>
					...
					<template #montoBruto="{ item }">
						<td class="text-end">
							{{ formatPrice(item.montoBruto) }}
						</td>
					</template>

					<template #montoRetenciones="{ item }">
						<td class="text-end">
							{{ formatPrice(item.montoRetenciones) }}
						</td>
					</template>

					<template #montoNeto="{ item }">
						<td class="text-end">
			...
			<SelectBeneficiario
				:isVisible="showBeneficiarioModal"
				@close="closeBeneficiarioModal"
			/>
			<FormularioCodificacionGastoDialog
				@saveDetalle="saveDetalle"
				:registroGasto="postRegistroGasto"
				:cuentaBanco="cuentaBanco"
				:showModal="ShowCodificacionGastoModal"
				@closeModal="closeCodificacionGasto"
			/>
		</CModalBody>
		<CModalFooter>
			<CButton
...
<script>
	...
	export default {
		...
		methods: {
			...
			deleteDetalle(item) {
				this.postRegistroGasto.detalleRegistroGastos.splice(
					this.postRegistroGasto.detalleRegistroGastos.indexOf(item),
					1,
				);
			},
			changeFormaPago(e) {
			...
			getCuentabanco(e) {
				this.cuentaBanco = Array.from(e.target).filter(
					(cuenta) => cuenta.value == e.target.value,
				)[0].label;
			},
			saveDetalle(payload) {
				if (payload.detalleRetencion.length > 0) {
					let montoNeto = payload.detalleRetencion.map(
						(detalle) => detalle.valorAplicado,
					);
					this.postRegistroGasto.montoNeto = montoNeto.reduce(
						function (a, b) {
							return a + b;
						},
					);

					this.postRegistroGasto.detalleRegistroGastos = [
						payload,
						...this.postRegistroGasto.detalleRegistroGastos,
					];
			...
			closeBeneficiarioModal(payload) {
				if (payload.grupoBeneficiarioId) {
					...
					this.postRegistroGasto.grupoCompensacionId =
						payload.grupoBeneficiarioId;
					this.postRegistroGasto.beneficiarioId =
		...
		watch: {
			postGasto(newValue) {
				if (newValue) {
					this.postRegistroGasto = {
						...newValue,
						conceptoGastoId: newValue.conceptoGasto.id,
						bancoId: newValue.banco.id,
						beneficiarioId: newValue.beneficiario.id,
					};
					this.displayBeneficiario =
						newValue.beneficiario.descripcion;
					this.cuentaBanco = newValue.banco.descripcion;

					this.postRegistroGasto.detalleRegistroGastos.map(
						(detalle) => {
							detalle.id = 0;
