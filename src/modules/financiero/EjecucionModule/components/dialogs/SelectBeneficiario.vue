			...
			<CTabContent>
				...
				>
					...
					<CSmartTable
						...
						tableFilter
						header
						:items="grupoBeneficiarios"
						:columns="grupoBeneficiariosColumns"
						itemsPerPageSelect
						:itemsPerPage="5"
						:items-per-page-options="[5, 10, 20]"
						columnSorter
							...
							<td class="py-2">
								<div class="d-flex justify-content-around">
									<CButton
										...
										size="sm"
										@click="
											selectBeneficiarioPorGrupo(item)
										"
									>
										Seleccionar</CButton
									>
...
<script setup>
	...
	const selectBeneficiarioPorGrupo = (item) => {
		Api.getGrupoCompensacionBeneficiario(item.id).then((response) => {
			closeDialog({
				grupoBeneficiarioId: item.id,
				beneficiarioId: response.data.data[0].id,
				beneficiarioNombre: item.descripcion,
			});
		});
	};
	...
	function autoSelect(term) {
		if (beneficiarios.value.length && term) {
			const found = beneficiarios.value.find(
				(beneficiario) =>
					beneficiario.id == term ||
					beneficiario.descripcion === term,
			);
			if (found) {
				closeDialog(found);
			}
			if (String(term).length >= 6 && !found) {
				toastStore.show({
					content: `Beneficiario ${term} no encontrado`,
					closable: false,
					color: 'warning',
					time: 5_000,
				});
