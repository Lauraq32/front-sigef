<template>
	...
	<AppAccionHeader :actions="pageActions">
		<CButton
			...
			>Agregar</CButton
		>
		<CButton color="secondary" @click="goToIngreso"
	...
	export default {
					...
					reader.onload = (e) => {
						...
						data.map((item) => {
							...
							proyectosList.push({
								...
								AyuntamientoId:
									this.LoginInfo.ayuntamientoId ?? 0,
								...
								Proyecto: `${Object.values(item)[4]
									.toString()
									.padStart(2, 0)}`,
								ActObra: `${Object.values(item)[5]
									.toString()
									.padStart(4, 0)}`,
								EstControl: `${Object.values(
									item,
								)[2].toString()}${Object.values(
									item,
								)[3].toString()}${Object.values(
									item,
								)[4].toString()}0000`,
						...
						if (proyectosList.length) {
							Api.postCargaMasivaCabecera(proyectosList)
								.then(() => {
									this.show({
										content:
				...
				if (this.file) {
					...
					reader.onload = (e) => {
						...
						data.map((item) => {
							if (Object.values(item)[2] < 90) {
								this.pnap = '00';
								this.programa = Object.values(item)[2]
									.toString()
									.padStart(2, 0);
							} else if (Object.values(item)[2] > 90) {
							...
							pregastoMasivo.push({
								...
								estimadoBco1: 0,
								presupuestoBco1: `${
									Object.values(item)[11] == 'P'
										? Object.values(item)[18]
										: 0
								}`,
								variacionBco1: 0,
						...
						if (pregastoMasivo.length) {
							Api.postCargaMasivaDetalle(pregastoMasivo)
								.then(() => {
									this.show({
										content:
											'Registros añadido correctamente',
										closable: true,
									});
									this.loadData();
								})
						...
						...[
							{
								...
								accionHandler:
									this.cargarEstructuras.bind(this),
								icon: 'cilLayers',
							},
							{
								label: 'Importar Proyectos',
								accionHandler:
									this.onFileChangeProyectos.bind(this),
								type: 'upload',
							},
							{
		...
		watch: {
			prepGastoList() {
				setTimeout(() => {
					this.footerItem[0].label = `Total items: ${this.gastoListCount}`;
					this.footerItem[1].label = this.formatPrice(
						this.totalBugetAmount,
					);
				}, 200);
			},
		},
