<template>
	...
	<AppAccionHeader :actions="pageActions">
		<CButton color="info" @click="openModal" v-if="!isFiscalYearClosed"
			>Agregar</CButton
		>
		<CButton color="secondary" @click="goToGasto"
	...
	export default {
		...
		methods: {
			...
			downloadFile() {
				Api.downloadIngreso()
					.then((response) => {
						...
						const id = `${
							this.$loggedInfo.user.ayuntamiento.codigo
						}-${new Date().toISOString()}`;

						fURL.href = fileURL;
						fURL.id = id;
						fURL.setAttribute('download', `FI-${id}.csv`);
			...
			onFileChange(event) {
				...
				if (this.file) {
					...
					reader.onload = (e) => {
						...
						Api.postCargaMasiva(presIngrsoMasivo)
							.then(() => {
								this.show({
			...
			getTotales() {
				Api.getTotalIngresos(
					...
					this.footerItem[1].label = this.formatPrice(
						response.data.data.anioAnt,
					);
					this.footerItem[2].label = this.formatPrice(
						response.data.data.alaFecha,
					);
					this.footerItem[3].label = this.formatPrice(
						response.data.data.presForm,
					);
				});
			},
			...
			submitForm(inputData) {
				let accion = null;
				if (inputData.id) {
					accion = Api.editPresIngreso(inputData.id, inputData).then(
						(response) => {
							this.show({
					...
					accion = Api.postFormulacionIngreso(inputData).then(
						(response) => {
							this.show({
