  ...
  methods: {
    ...
    submitForm() {
      if (this.id) {
        ...
        setTimeout(this.getArea, 500)
        Api.postArea(this.postAreaUbicacion).then((response) => {
          this.$swal({
            position: 'top-end',
            icon: 'success',
            title: response.data.message,
...
export default {
  ...
  methods: {
    ...
    toggleDetails(item) {
      ...
      console.log(item.id)
      Api.getAreaByID(item.id).then((response) => {
        this.postAreaUbicacion = response.data
        console.log(response)
        this.id = item.id
        //this.postIngreso = response.data.data
      })
...
export default {
  ...
  setup() {
    ...
    const { getArea, areaUbicacion } = store
    ...
    return {
      ...
      getBadge,
      columns,
      areaUbicacions: computed(() => store.areaUbicacion),
    }
  },
}
</script> -->
